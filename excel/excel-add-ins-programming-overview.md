# Excel アドインの JavaScript プログラミングの概要

_適用対象: Excel 2016、Office 2016_

この記事では、Excel 2016 で JavaScript API を使用してアドインをビルドする基本的な方法について説明します。Excel JavaScript API の仕様について詳しくは、[リファレンス](excel-add-ins-javascript-reference.md)のページをお読みください。

## 基本事項

最初に、RequestContext、JavaScript プロキシ オブジェクト、sync()、Excel.run()、load() などの API を使用するために知っておくべき主な概念を簡単に紹介します。セクションの末尾にあるコード例で、これらの概念をどのように使用するかを示しています。


#### RequestContext

RequestContext オブジェクトは、Excel アプリケーションへの要求を容易にします。Office アドインと Excel アプリケーションは 2 つの異なるプロセスで実行されているため、アドインから Excel やその関連オブジェクト (ワークシート、テーブルなど) にアクセスするには、要求のコンテキストが必要となります。要求のコンテキストは次のように作成されます。
...
* `properties` は、読み込まれるプロパティや関係の名前の一覧で、名前のコンマ区切りの文字列または配列として指定されます。詳細は、各オブジェクトの下の .load() メソッドをご参照ください。
* `loadOption` は、selection、expansion、top、skip の各オプションについて説明するオブジェクトを指定します。詳細は、オブジェクトの読み込みの[オプション](resources/loadoption.md)をご覧ください。

##### 例

次の例で、上記の概念をまとめます。このサンプル コードは、配列から範囲オブジェクトへの値の書き込みを示しています。 
...
既定では、object.load() は、読み込まれるオブジェクトのスカラー プロパティと複合プロパティをすべて選択します。リレーションシップは既定では読み込まれません (たとえば、書式は 範囲オブジェクトのリレーションシップ オブジェクトです)。しかし、パフォーマンスの向上のためには、常にプロパティと関係が読み込まれるように明示的にマークすることをお勧めします。これを行うには、(`load()` パラメーターで) プロパティとリレーションシップのサブセットを応答に含めるように指定します。load メソッドには、2 種類の入力方式があります。すなわち、

* プロパティとリレーションシップの名前をコンマ区切りの文字列の名前として入力する、_または_プロパティやリレーションシップの名前を含んだ文字列の配列として入力することができます。 
* selection、expansion、top、skip の各オプションについて説明するオブジェクト。詳細は、オブジェクトの読み込みの[オプション](resources/loadoption.md)をご覧ください。
...
以下の load ステートメントでは、範囲のすべてのプロパティを読み込んでから、書式と塗りつぶしの書式を拡張します。  
...
次の設定要求では、範囲内のある部分の番号書式のみを設定し、残りの部分では (null 値を渡すことで) 既存の番号書式を保持しています。
...
広い範囲とは、1 つの API の呼び出しに対してサイズが大きすぎる範囲を意味します。範囲に含まれる、セル数、値、番号書式、数式などの多くの要因により、応答のサイズが大きくなりすぎて API での操作に適さなくなることがあります。API は、要求されたデータを返したり、それに書き込んだりしようと最善を尽くします。しかし、大きなサイズが関係していると、リソース使用率が大きくなるために、API エラー状態になることがあります。 

このような状態にならないようにするため、広い範囲の読み取りや書き込みは、よりサイズの小さい複数の範囲に分けて行うことをお勧めします。
