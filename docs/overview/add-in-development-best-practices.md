...
- [魅力的な初回実行時エクスペリエンス](#魅力的な初回実行時エクスペリエンス)を作成して、ユーザーがアドインの価値をすぐに感じられるようにしてください。
...
- Office リボンを拡張するアドイン:
  ...
  - アドインの使用スペースを増やす余分なボタンを追加しないでください。
     ...
     >  **注**  使用スペースが大きすぎるアドインは、 [Office ストアの検証](https://msdn.microsoft.com/library/cd90836a-523e-42f5-ab02-5123cdf9fefe(Office.15).aspx).
...
- すべてのプラットフォームと入力方法 (マウスやキーボード、および [タッチ](#タッチ)など) に対応するように設計してください。UI が様々なフォーム ファクターに対応するようにしてください。
