...
Outlook アドインは XML アドイン マニフェストと Web ページの 2 つのコンポーネントで構成されています。これらは Office アドイン (office.js) の JavaScript ライブラリでサポートされます。マニフェストは、アドインが Outlook クライアント間でどのように統合されるかを説明します。次に例を示します。
 ...
 >**注**  次のサンプルの URL 値はすべて "https://appdemo.contoso.com" で始まります。この値はプレースホルダーです。実際に有効なマニフェストでは、これらの値に有効な HTTPS Web URL が含まれます。
...
この要素では、アドインによる[モバイル アドイン](./add-mobile-support.md)のサポートも定義します。
...
> **注:**アクティブ化ルールは、**VersionOverrides** 要素をサポートしないクライアントにのみ適用されます。 
...
アドイン コマンドを定義するアドインの例については、「[コマンド デモ](https://github.com/jasonjoh/command-demo)」をご覧ください。
