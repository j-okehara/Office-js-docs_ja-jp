...
# <a name="customxmlparts.getbynamespaceasync-method"></a>CustomXmlParts.getByNamespaceAsync メソッド
指定された名前空間に一致するカスタム XML パーツの配列を非同期的に取得します。
...
**getByNamespaceAsync** メソッドに渡されたコールバック関数で、**AsyncResult** オブジェクトのプロパティを使用し、次の情報を返すことができます。



|**プロパティ**|**使用目的**|
|:-----|:-----|
|[AsyncResult.value](../../reference/shared/asyncresult.value.md)|指定の名前空間に一致する **CustomXMLPart** オブジェクトの [array](../../reference/shared/customxmlnode.getnodesasync.md) にアクセスします。|
