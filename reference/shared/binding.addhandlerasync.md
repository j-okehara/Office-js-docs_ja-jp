...
# <a name="binding.addhandlerasync-method"></a>Binding.addHandlerAsync メソッド
指定されたイベントの種類のバインドにハンドラーを追加します。
...
|**名前**|**型**|**説明**|**サポートのメモ**|
|:-----|:-----|:-----|:-----|
| _eventType_|[EventType](../../reference/shared/eventtype-enumeration.md)|追加するイベントの種類を指定します。必須です。 **Binding** オブジェクト イベントの場合、 _eventType_ パラメーターには、 **Office.EventType.BindingSelectionChanged**、 **Office.EventType.BindingDataChanged**、またはこれらの列挙型の対応するテキスト値を指定できます。||
| _handler_|**object**|追加するイベント ハンドラー関数。||
...
次のコード例では、[Office](../../reference/shared/office.select.md) オブジェクトの **select** メソッドを呼び出して、"MyBinding" という ID を持つバインドにアクセスします。次に、**addHandlerAsync** メソッドを呼び出して、そのバインドの [bindingDataChanged](../../reference/shared/binding.bindingdatachangedevent.md) イベントのハンドラー関数を追加します。
