...
# <a name="document.getfilepropertiesasync-method"></a>Document.getFilePropertiesAsync メソッド
...
**getFilePropertiesAsync** メソッドに渡されるコールバック関数で、**AsyncResult** オブジェクトのプロパティを使用して、次の情報を返すことができます。



|**プロパティ**|**使用目的**|
|:-----|:-----|
|[AsyncResult.value](../../reference/shared/asyncresult.value.md)|ファイルの URL を  `asyncResult.value.url` に返します。|
...
このリリースの API では、 **url** プロパティ ( `asyncResult.value.url`) を使用してファイルの URL の取得のみができます。


## <a name="example"></a>例

現在のファイルの URL を読み取るには、URL を返すコールバック関数を記述する必要があります。次の例は、その方法を示しています。


 - ファイルの URL の値を返す **匿名のコールバック関数を** _getFilePropertiesAsync_ メソッドの **callback**パラメーターに渡します。
