...
# <a name="customxmlnode.getnodesasync-method"></a>CustomXmlNode.getNodesAsync メソッド
相対 XPath 式と一致する **CustomXMLNode** オブジェクトの配列としてノードを取得します。
...
|**名前**|**型**|**説明**|**サポートのメモ**|
|:-----|:-----|:-----|:-----|
| _xPath_|**string**|取得するノードを指定する XPath 式。必須。||
...
**getNodesAsync** メソッドに渡されるコールバック関数では、**AsyncResult** オブジェクトのプロパティを使用して、次の情報を返すことができます。



|**プロパティ**|**使用目的**|
|:-----|:-----|
|[AsyncResult.value](../../reference/shared/asyncresult.value.md)|**xPath** パラメーターに渡される XPath 式で指定されたノードを表す [CustomXMLNode](../../reference/shared/customxmlnode.customxmlnode.md) オブジェクトの _array_ にアクセスします。|
