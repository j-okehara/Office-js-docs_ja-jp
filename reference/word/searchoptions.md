...
|matchWildCards|bool|特殊な検索演算子を使用して検索を実行するかどうかを示す値を取得または設定します。[検索と置換] ダイアログ ボックスの [ワイルドカードを使用する] チェック ボックスに相当します。このオプションの使用に関する重要な情報については、下のワイルドカードのガイダンスを参照してください。|
